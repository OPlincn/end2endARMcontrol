...
void MobileSamNode::RosImgProcess(
  ...
  try {
    if ("rgb8" == img_msg->encoding) {
      ...
      // --- 增加亮度 ---
      // alpha: 对比度 (1.0 表示不变)
      // beta: 亮度增量 (正值增加亮度，负值降低亮度)
      double alpha = 1.0; 
      double beta = 50.0; // 可根据需要调整此值
      cv_img->image.convertTo(cv_img->image, -1, alpha, beta);
      // --- 亮度调整结束 ---
    ...
    RCLCPP_ERROR(rclcpp::get_logger("mono_mobilesam"), "Exception: %s", e.what());
    return;
  }
